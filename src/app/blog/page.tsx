  ...
  return (
    <div style={{ background: 'var(--cream)', minHeight: '100vh' }}>
      <style>{`
        ...
        .blog-card-img { width: 100%; height: 180px; object-fit: cover; border-radius: 12px 12px 0 0; display: block; }
        .blog-card-img-placeholder { width: 100%; height: 180px; background: linear-gradient(135deg, #FFE0CC 0%, #FFCBA4 100%); border-radius: 12px 12px 0 0; display: flex; align-items: center; justify-content: center; font-size: 40px; }
      `}</style>
      ...
      <div className="blog-grid">
        ...
        <Link href={`/blog/${featured.slug}/`} style={{ textDecoration: 'none', display: 'block', marginBottom: 40 }}>
          <article className="blog-featured-hover" style={{
            ...
            {/* Decoration */}
            {!featured.imageUrl && (
              <svg style={{ position: 'absolute', top: 20, right: 60, opacity: 0.3, zIndex: 1 }} width="60" height="60" viewBox="0 0 60 60" aria-hidden="true">
                <path d="M30 0 Q33 27 60 30 Q33 33 30 60 Q27 33 0 30 Q27 27 30 0Z" fill="#FFC547"/>
              </svg>
            )}

            <div style={{ position: 'relative', zIndex: 1, padding: 'clamp(28px, 4vw, 48px)' }}>
              ...
              <article className="blog-card-hover" style={{
                ...
                {post.imageUrl ? (
                  ...
                  <div className="blog-card-img-placeholder">
                    {post.category === 'Советы родителям' ? '👨‍👩‍👧' : post.category === 'Типы школ' ? '🏫' : '📊'}
                  </div>
                )}
