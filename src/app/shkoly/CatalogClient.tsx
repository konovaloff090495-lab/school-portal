...
export default function CatalogClient({
  ...
  useEffect(() => {
    ...
    // From /shkoly/moskva/{type}/ → 1 district selected → rayon/{district}/{type}/
    if (lockRegion && lockType && initialRegions[0] === 'moskva' && !initialDistrict
  ...
  const applyBaseFilters = useCallback((
    ...
    if (skip !== 'price') {
      if (filters.priceCategories.length) {
        ...
        if (filters.priceMode === 'free') list = list.filter(s => (s.priceFrom === 0 || s.priceFrom === undefined) && s.type !== 'podgotovka-ege' && s.type !== 'podgotovka-oge' && s.type !== 'programmirovanie' && s.type !== 'shahmatnye')
