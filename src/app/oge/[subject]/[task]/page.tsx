...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const task = getOgeTask(subjectSlug, taskSlug)
  if (!subject || !task) return {}
  return {
    title: `ОГЭ ${subject.nameShort} — ${task.title}: ${task.shortDesc} | pro-schools.ru`,
    description: `Разбор ${task.title} ОГЭ по ${subject.name}: что проверяет, какие темы нужно знать, примеры с решением и тренажёр. Максимальный балл — ${task.maxScore}.`,
    keywords: `${task.title} ОГЭ ${subject.nameShort.toLowerCase()}, ${task.shortDesc.toLowerCase()} ОГЭ`,
    alternates: { canonical: `https://pro-schools.ru/oge/${subjectSlug}/${taskSlug}/` },
  }
}

export default async function OgeTaskPage({ params }: Props) {
  const { subject: subjectSlug, task: taskSlug } = await params
  const subject = getOgeSubject(subjectSlug)
  const task = getOgeTask(subjectSlug, taskSlug)
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-4xl mx-auto px-4 py-6">
          ...
          <nav className="flex items-center gap-2 text-sm text-gray-400 mb-4 flex-wrap">
            ...
            <span>/</span>
            <Link href={`/oge/${subjectSlug}/`} className="hover:text-gray-600">{subject.name}</Link>
            <span>/</span>
            <span className="text-gray-700 font-medium">{task.title}</span>
          </nav>

          <div className="flex items-start gap-4">
            <div className="w-14 h-14 rounded-2xl bg-blue-100 flex items-center justify-center text-2xl font-black text-blue-600 flex-shrink-0">
              {task.number}
            </div>
            <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="flex items-center justify-between mt-6 gap-4">
              {prevTask ? (
                <Link
                  href={`/oge/${subjectSlug}/${prevTask.slug}/`}
                  className="flex items-center gap-2 bg-white border border-gray-100 hover:border-blue-200 rounded-xl px-4 py-3 transition-colors group flex-1 min-w-0"
                >
                  <span className="text-gray-400 group-hover:text-blue-500 transition-colors">←</span>
                  <div className="min-w-0">
                    <p className="text-xs text-gray-400">Предыдущее</p>
                    <p className="text-sm font-semibold text-[#0F172A] group-hover:text-blue-600 truncate">{prevTask.title}</p>
                  </div>
                </Link>
              ...
              {nextTask ? (
                <Link
                  href={`/oge/${subjectSlug}/${nextTask.slug}/`}
                  className="flex items-center gap-2 bg-white border border-gray-100 hover:border-blue-200 rounded-xl px-4 py-3 transition-colors group flex-1 min-w-0 text-right"
                >
                  <div className="min-w-0 flex-1">
                    <p className="text-xs text-gray-400">Следующее</p>
                    <p className="text-sm font-semibold text-[#0F172A] group-hover:text-blue-600 truncate">{nextTask.title}</p>
                  </div>
                  <span className="text-gray-400 group-hover:text-blue-500 transition-colors">→</span>
                </Link>
              ) : <div className="flex-1" />}
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-4">
              <h3 className="text-sm font-bold text-[#0F172A] mb-3">
                Все задания ОГЭ по {subject.nameShort}
              </h3>
              <div className="space-y-1">
                {subject.tasks.map(t => (
                  <Link
                    key={t.slug}
                    href={`/oge/${subjectSlug}/${t.slug}/`}
              ...
              <Link
                href={`/oge/${subjectSlug}/`}
                className="block text-center text-xs text-blue-600 hover:text-blue-700 font-semibold mt-3 pt-3 border-t border-gray-50"
              >
                ← К списку заданий
              </Link>
            </div>
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 rounded-2xl p-4">
              ...
              <p className="text-xs text-gray-500 mb-3">Найдите школу с углублённой подготовкой к ОГЭ.</p>
              <Link
                href="/shkoly/osobennosti/podgotovka-k-oge/"
                className="block text-center bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-4 py-2 rounded-lg transition-colors"
              >
                Найти школу →
              </Link>
            </div>
