...
import { egeSubjects, getEgeSubject } from '@/data/ege-oge'

interface Props { params: Promise<{ subject: string }> }

export function generateStaticParams() {
  return egeSubjects.map(s => ({ subject: s.slug }))
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!subject) return {}
  return {
    title: `ЕГЭ по ${subject.name} 2025 — все задания с разбором | pro-schools.ru`,
    description: `Подготовка к ЕГЭ по ${subject.name}: разбор всех ${subject.totalTasks} заданий, теория, формулы, примеры и тренажёры. ${subject.description}`,
    keywords: `ЕГЭ ${subject.nameShort.toLowerCase()}, задания ЕГЭ ${subject.nameShort.toLowerCase()}, подготовка к ЕГЭ ${subject.nameShort.toLowerCase()}`,
    alternates: { canonical: `https://pro-schools.ru/ege/${subjectSlug}/` },
  }
}

export default async function EgeSubjectPage({ params }: Props) {
  const { subject: subjectSlug } = await params
  const subject = getEgeSubject(subjectSlug)
  if (!subject) notFound()

  // Разделяем задания на часть 1 и часть 2 (часть 2 обычно требует развёрнутого ответа, maxScore > 1)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-5xl mx-auto px-4 py-6">
          ...
          <div className="flex items-start gap-4">
            <div className="w-14 h-14 rounded-2xl bg-violet-50 flex items-center justify-center text-3xl flex-shrink-0">
              {subject.icon}
            </div>
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs bg-violet-100 text-violet-700 font-bold px-2 py-0.5 rounded-full">ЕГЭ</span>
                <span className="text-xs text-gray-400">11 класс</span>
              </div>
              <h1 className="text-2xl md:text-3xl font-black text-[#0F172A]">ЕГЭ по {subject.name}</h1>
          ...
          <div className="flex flex-wrap gap-4 mt-5">
            {[
              ...
              <div key={item.label} className="bg-gray-50 rounded-xl px-4 py-2 text-center">
                <div className="text-lg font-black text-violet-600">{item.value}</div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
          ...
          <div className="mb-10">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 divide-y divide-gray-50 overflow-hidden">
              {part1.map((task, idx) => (
                <Link
                  key={task.slug}
                  href={`/ege/${subjectSlug}/${task.slug}/`}
                  className="flex items-center gap-4 px-5 py-4 hover:bg-violet-50 transition-colors group"
                >
                  <div className="w-9 h-9 rounded-lg bg-violet-50 group-hover:bg-violet-100 flex items-center justify-center text-sm font-black text-violet-600 flex-shrink-0 transition-colors">
                    {task.number}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-[#0F172A] group-hover:text-violet-600 transition-colors">
                      {task.title}
                    </p>
        ...
        {/* Если задания ещё не добавлены — показываем заглушку */}
        {subject.tasks.length === 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 p-10 text-center">
            <p className="text-4xl mb-4">🚧</p>
            <h3 className="font-bold text-[#0F172A] mb-2">Разборы скоро появятся</h3>
            <p className="text-sm text-gray-500">Готовим подробный разбор каждого задания ЕГЭ по {subject.name}.</p>
          </div>
        )}

        {/* Навигация к школам с подготовкой */}
        <div className="mt-8 bg-gradient-to-br from-violet-50 to-purple-50 border border-violet-100 rounded-2xl p-6 flex items-center justify-between gap-4 flex-wrap">
          <div>
            <h3 className="font-black text-[#0F172A] text-base mb-1">Нужна школа с подготовкой к ЕГЭ?</h3>
