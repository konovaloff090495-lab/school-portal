...
def main():
    ...
    for it in d['items']:
        ...
        # Арсентьев 8: «Стр.103 Материал для самостоятельной работы» / «Общественная мысль. Материал для самостоятельной работы»
        ms = re.match(r'^(.*?)\s*Материал для самостоятельной работы', it['h1'])
        if ms and kind == 'par':
            topic = ms.group(1).strip().rstrip('.')
            topic = re.sub(r'^Стр\.\s*(\d+)$', r'стр. \1', topic)
            title = f'Материал для самостоятельной работы к § {n}' + (f': {topic}' if topic else '')
            mp = None
        if mp:
    ...
    def ch_sort(t):
        ...
        m = re.match(r'Материал для самостоятельной работы к § (\d+)(?::\s*(.*))?', t)
        if m:
            return (0, int(m.group(1)), 1, m.group(2) or '')
