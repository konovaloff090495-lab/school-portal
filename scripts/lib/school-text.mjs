...
export const TYPE_META = {
  ...
  // Шахматные школы в реестре — это учреждения дополнительного образования
  // (ДЮСШ/СШОР по шахматам), а не общеобразовательные. Поэтому у них свои
  // формулировки про возраст и оплату: про ФГОС и «основные предметы» тут нельзя.
  shahmatnye:       { label: 'шахматная школа',       grades: '1–11', priceFrom: 0,     features: ['Занятия для детей и подростков', 'Турнирная практика', 'Тренеры по шахматам', 'Выполнение разрядных норм', 'Группы для начинающих'],
                      gradesText: 'Занимаются дети и подростки школьного возраста — от начинающих до разрядников.' },
...
const SHORT = {
  ...
  shahmatnye: c => [
    `Шахматная школа ${c} — занятия с тренерами и турнирная практика.`,
    `Шахматная школа ${c}: обучение детей и подростков, от первых правил до разрядов.`,
    `Шахматная школа ${c} — подготовка к турнирам и выполнение разрядных норм.`,
  ],
  programmirovanie: c => [
...
const CLOSING = {
  ...
  shahmatnye:       'Это учреждение дополнительного образования: занятия по шахматам идут помимо основной школы, ученики выступают на турнирах и выполняют разрядные нормы.',
...
export function makeDescriptions(name, type, cityName, cityIn, address, meta = TYPE_META[type]) {
  ...
  // gradesText/moneyText — переопределения для типов, к которым дефолтные
  // формулировки неприменимы (напр. допобразование: там нет ФГОС и классов)
  const grades = meta.gradesText ?? `Обучение ведётся с ${from} по ${to ?? from} класс.`

  const money = meta.moneyText !== undefined
    ? meta.moneyText
    : meta.priceFrom > 0
