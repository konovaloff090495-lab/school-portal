...
// --kind=korrektsionnye|kadetskie — какой вид школ добираем из реестра
const KIND = String(args.kind || 'korrektsionnye')
if (!['korrektsionnye', 'kadetskie'].includes(KIND)) { console.error('нужен --kind=korrektsionnye|kadetskie'); process.exit(1) }

// Общий шум: не школы. Для кадетских дополнительно режем СПО/училища/суворовские
// (они федеральные, Минобороны — в реестре лицензий как школы не проходят и это
// отдельный формат, не наш kadetskie-каталог).
const NOT_SCHOOL_BASE = 'ДОШКОЛЬН|ДЕТСК.{0,3}САД|ДОПОЛНИТЕЛЬН.{0,20}ПРОФЕССИ|ПРОФЕССИОНАЛЬН|\\bДПО\\b|ВЫСШЕГО ОБРАЗОВ|УНИВЕРСИТЕТ|ИНСТИТУТ|АКАДЕМИЯ|КОЛЛЕДЖ|ТЕХНИКУМ|АВТОШКОЛ'

const KIND_CFG = {
  korrektsionnye: {
    // eoName ищет по подстроке названия. Современный термин в реестре — «адаптивная
    // школа» (коррекционные массово переименованы); старое «коррекционн» осталось
    // не везде. «Специальная школа открытого/закрытого типа» — это СУВУ для девиантных
    // подростков, НЕ наш вид: их отсекает RELEVANT (нет коррекц/овз/адаптивн).
    QUERIES: ['адаптивн', 'коррекционн', 'для обучающихся с ограниченными', 'адаптированных образовательных'],
    // имя должно прямо указывать на коррекционный вид / ОВЗ, иначе это обычная школа
    RELEVANT: /коррекц|ограниченными возможностями|\bовз\b|адаптивн|для обучающихся с (?:умственн|задержк|наруш|тяжел|расстройств)|глухих|слабослышащ|позднооглох|слепых|слабовидящ|тяжелыми нарушениями речи|нарушениями опорно|задержкой психического|умственно отсталы/i,
    NOT_SCHOOL: new RegExp(NOT_SCHOOL_BASE + '|УЧИЛИЩ|СПОРТИВН|САНАТОРН|ДОМ РЕБЕНКА|РЕАБИЛИТАЦ|ПОМОЩИ ДЕТ', 'i'),
  },
  kadetskie: {
    QUERIES: ['кадетск', 'казач', 'кадетский корпус'],
    RELEVANT: /кадетск|кадетский корпус|казач[а-я]*\s+кадет|президентск[а-я]*\s+кадет|мореходн[а-я]*\s+кадет/i,
    // суворовские/нахимовские — федеральные МО, не наш формат; режем
    NOT_SCHOOL: new RegExp(NOT_SCHOOL_BASE + '|УЧИЛИЩ|СУВОРОВ|НАХИМОВ|\\bСПО\\b', 'i'),
  },
}
const QUERIES = KIND_CFG[KIND].QUERIES
const RELEVANT = KIND_CFG[KIND].RELEVANT
const NOT_SCHOOL = KIND_CFG[KIND].NOT_SCHOOL
...
const KIND_LABEL = { korrektsionnye: 'Коррекционная школа', kadetskie: 'Кадетская школа' }
// срезаем ведущие аббревиатуры организационно-правовой формы (МАОУ, ГБОУ, ГКОУ,
// ФГКОУ, КОУ, ЧОУ, АНОО…) — иначе они лезут в человекочитаемое имя
const OPF_ABBR = /^(?:[А-ЯЁ]{0,2}(?:МА?О?У|Б?ОУ|Г?К?ОУ|К?ОУ)|ФГ?К?ОУ|МБОУ|МКОУ|МАОУ|ГБОУ|ГКОУ|ГОУ|КОУ|КГ?О?У|ЧОУ|АНОО?|НОУ|ОГ?К?ОУ|БОУ|ГБПОУ)\b[\s"«]*/i
function stripOPF(s) { let p = s; for (let i = 0; i < 3; i++) { const n = p.replace(OPF_ABBR, ''); if (n === p) break; p = n } return p.trim() }
function displayName(short, full) {
  const src = [short, full].filter(Boolean).map(x => stripOPF(x.replace(/\s+/g, ' ').trim()))
  if (!src.length) return null
  const joined = src.join(' ')
  const num = (joined.match(/№\s*(\d+[А-Яа-яA-Za-z]?)/) || [])[1]
    || (joined.match(/\bШКОЛА\s+(\d{1,3})\b/i) || [])[1] || null
  const kind = KIND_LABEL[KIND]

  // фирменное имя в кавычках, если это не аббревиатура
  const quoted = [...joined.matchAll(/[«"]([^«»"]{4,70})[»"]/g)].map(m => m[1].trim())
    .filter(q => /[аеёиоуыэюя]/i.test(q))
    .filter(q => q.split(/\s+/).every(w => !/^[А-ЯЁ]{3,}$/.test(w)) || /[а-яё]{4,}/.test(q))
  if (quoted.length) {
    const q = quoted[quoted.length - 1]
    return /школ|гимназ|лице|корпус|интернат|центр|образован/i.test(q) ? titleCase(q) : `${kind} «${titleCase(q)}»`
  }
  // разворачиваем полное название, срезая организационно-правовую шелуху
  const spelled = src.find(x => /коррекц|кадетск|интернат|для обучающихся|казач|овз/i.test(x)) || src[0]
  const n = spelled
    .replace(/^(муниципальное|государственное|краевое|областное|казенное|казённое|бюджетное|автономное|общеобразовательное|образовательное|специальное|специальная|учреждение|организация|среднее|средняя|основное|основная|казенное общеобразовательное)\s+/gi, '')
    .replace(/\s{2,}/g, ' ').trim()
