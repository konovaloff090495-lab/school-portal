...
- Старые `/shkoly/<school-slug>/` перенаправляются только при точном совпадении с существующей карточкой (206 примеров GSC в scripts/seo/legacy-school-paths.json). Валидные города и неизвестные школы не затрагиваются. В учебнике числовые классы 1–11 нормализуются в `N-klass`.
